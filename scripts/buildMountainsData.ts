...
async function main() {
  ...
  for (const mountain of missingMountains) {
    const outFileName = `${mountain.id}.webp`;
    const outPath = path.join(OUT_DIR, outFileName);
    
    let imageUrl = '/images/mountains/placeholder.webp';
    try {
      await fs.access(outPath);
      imageUrl = `/images/mountains/${outFileName}`;
    } catch {
      // file not found
    }
    ...
    const entry = `
  {
    ...
    imageUrl: '${imageUrl}',
    imageCredit: '${imageUrl.includes('placeholder') ? 'Unknown' : 'Wikimedia Commons'}',
  },`;
  ...
  if (insertionIndex !== -1) {
    ...
    console.log(`✅ Done! Data added to mountains.ts without syntax error.`);
  } else {
    console.error('❌ Could not find proper insertion point in mountains.ts');
  }
}
